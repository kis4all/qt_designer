...
inspection_start = False
inspection_check = False
inspection_check_done = False
result_final_rpm = 0
rpm_max = 2940
rpm_min = 1360
set_current_min = 1.0
...
class Inspection(QObject):
    ...
    def run(self):
        ...
        global inspection_start
        global inspection_check
                    ...
                    try:
                        while 1:
                            ...
                            if (inspection_check == True) and (inspection_check_done == False) :
                                ...
                                if result_final_rpm > rpm_min and result_final_rpm < rpm_max :
                                    self.final_okng.emit(1)
                                else:
                                    self.final_okng.emit(0)
...
class Read_PLC(QObject):
    ...
    def run(self):
        ...
        while True:
            read_plc = client.read_coils(2000, 6, unit=MODBUS_ADDRESS)
            plc_ready = read_plc.bits[0]
            inspection_start_signal = read_plc.bits[1]
            inspection_check_signal = read_plc.bits[2]
            if plc_ready == 1 :
                print("plc_ready")
                self.io_progress.emit(1)
                if inspection_start_signal == True :
                    print("inspection_start")
                    self.io_progress.emit(2)
                    break
                if inspection_check_signal == True :
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def start_rpm(self):
        ...
        self.inspection.final_okng.connect(self.final_okng_update_save)
    ...
    def final_okng_update_save(self, final_okng ):
        if final_okng == 1:
            self.label_final_okng.setText('OK')
        else if final_okng == 0:
            self.label_final_okng.setText('NG')
        save_data()
    ...
    def data_save():
        ...
        rpm = str(self.label_final_rpm.text())+ '\t'
        current = str(self.label_final_current.text()) + '\t'
        ok_ng = str(self.label_okng.text()) + '\t'
